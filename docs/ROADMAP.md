...
**수용 기준** TTS 종료 직후 100 ms 안에 시작된 발화가 다음 턴 STT 스트림 앞부분에
포함된다. `turn_tone`이 켜진 구성에서는 톤 완료 시 링을 비우므로 이 기준은 톤이
꺼진(기본) 구성에서만 성립하고, 발화는 비프 이후에 시작한다.
...
### [x] P5-2. README 포지셔닝 재작성

**근거** §1.4, §4-1. 첫 문단이 기술 설명만 있고 core `voip`과의 차이를 말하지 않았다.

**작업**
1. README 상단에 core `voip` 대비 표를 넣었다: PBX에 REGISTER, G.711·G.722(Opus
   불필요), ATA 불필요, 발신·IVR·녹음.
2. 대표 시나리오 3개(전화로 HA 제어, 인터폰 자동응답, 센서 → 전화+TTS)를 결과
   중심으로 보여 주고 기존 YAML 절로 링크했다.
3. usage 배지를 내렸다. 2026-09-12 analytics `custom_integrations.json`에 `sip`
   도메인이 없다(4248개). 집계되면 되돌린다.
4. [Security] 섹션: 외부 노출 미지원(UDP only, TLS/SRTP 없음). 도어락은 P4-1
   권장 구성(`sip.start_assist` 화이트리스트+PIN+전용 pipeline)으로 링크.

**수용 기준** 첫 화면이 core `voip`과 다른 이유를 말하고, 외부 노출을 미지원으로
명시한다.
