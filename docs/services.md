...
`sip.start_assist` can follow `sip.dial` or `sip.answer` on the same call. If that call was started with a `message` or `menu`, the pending IVR (and its `post_action`) is retired when Assist takes over, so Assist's replies are not cut short by the announcement's `hangup`. While a `pin` is being collected the IVR is only paused: a rejected PIN resumes it (an announcement that finished during the prompt then runs its `post_action`), so a `message` can serve as the PIN prompt. To hand a menu to Assist from inside the IVR instead, use the [`assist` menu field](examples.md#menu-fields), which accepts the same tuning options.
...
- `interrupt_media` *(Optional)*: Stop and discard whatever is playing on the line (hold music, a `play_media` announcement) the moment Assist is ready — when it opens the microphone for the first turn, or when the first chunk of its `initial_prompt` reply is ready, whichever comes first (default: `true`). Until then the current media keeps playing, so you can cover pipeline / LLM / TTS warm-up with music. Without `initial_prompt` and `turn_tone`, "ready" is right at session start. Set `false` to keep the previous behavior: Assist waits (up to 60 s) for the current playback to finish before its first turn.
